...
- `docs/` — `RUST_ARCHITECTURE.md`, le guide d'archi Rust de référence.

## Contrat d'intégration

- L'app Flutter parle au backend Rust **uniquement via HTTP/REST** (`/api/v1/...`).
  Pas de FFI : deux process distincts.
- **Le backend est la source de vérité du contrat d'API.** Tout changement de
  route/DTO côté `rust/` doit être répercuté côté `app/` (et versionné via `/api/vN`).
